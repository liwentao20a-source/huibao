...
$$t_c^{\text{ref}} = \frac{H\sin\alpha}{V}, \quad R_{\text{ref}} = \sqrt{H^2\cos^2\alpha + y_c^2}$$

$$f_{dc} = f_d(0)\big|_{\text{ref}} = \frac{2V H\sin\alpha}{\lambda R_{\text{ref}}}$$

定义参考俯角 $\beta$：$\sin\beta = \dfrac{H}{R_{\text{ref}}}$（仅当 $y_c = 0$ 时严格成立；$y_c \neq 0$ 时应使用精确的 $\dfrac{H}{\sqrt{H^2 + y_c^2}}$），则：

$$\boxed{f_{dc} \approx \frac{2V\sin\alpha \cdot \sin\beta}{\lambda}} \tag{11}$$

**$y$ 依赖性**：$f_{dc}$ 通过 $R_{\text{ref}}$ 弱依赖于 $y_c$（在 $R_{\text{ref}}$ 的分母中）。对于典型几何（$y_c \ll H$），该依赖可忽略。但对大 $y$ 目标，多普勒中心会略有偏移。
...
在实际 SAR 处理中，常在波束中心时刻 $t = 0$ 处展开。对参考点 $(0, y_c, 0)$：

$$R(t) \approx R_{\text{ref}} - \frac{V H\sin\alpha}{R_{\text{ref}}} \cdot t + \frac{V^2(R_{\text{ref}}^2 - H^2\sin^2\alpha)}{2R_{\text{ref}}^3} \cdot t^2$$

线性距离走动：$\displaystyle R_{\text{walk}} = -\frac{V H\sin\alpha}{R_{\text{ref}}} \cdot t$
...
- 偏离方向沿 $(\sin\alpha, -\cos\alpha)$，即**垂直于等 $D$ 线的方向**（也即 $R_{\min}$ 的梯度方向——距离向）
...
令 $(23) = \pi/4$，并代入合成孔径时间 $T_a \approx \dfrac{\lambda |D|}{D_a V\cos\alpha}$（$D_a$ 为天线方位向尺寸）：

$$\frac{\pi V^2}{4\lambda} \cdot \frac{\lambda^2 D^2}{D_a^2 V^2\cos^2\alpha} \cdot \frac{y^2}{2|D|^3} = \frac{\pi}{4}$$

化简得：

$$y_{\max}^2 = \frac{2 |D| \cdot D_a^2 \cos^2\alpha}{\lambda}$$

$$\boxed{y_{\max} \approx D_a \cos\alpha \cdot \sqrt{\frac{2|D|}{\lambda}}} \tag{25}$$

**数值示例**：取 $D_a = 1\text{m}$，$\alpha = 10^\circ$，$|D| = 1000\text{m}$，$\lambda = 0.03\text{m}$（X波段）：

$$y_{\max} \approx 1 \times 0.985 \times \sqrt{\frac{2 \times 1000}{0.03}} \approx 254\text{m}$$

对于典型的近场目标（$y \sim 1\text{m} \ll 254\text{m}$），散焦效应**可忽略**。但对于远 $y$ 目标或高分辨率（大 $D_a$、短 $\lambda$）系统，散焦需考虑。

### 6.7 散焦的方位向展宽

二次相位误差导致方位向冲击响应展宽。定义展宽因子：

$$\gamma = \frac{\rho_{az}(y)}{\rho_{az}(0)} = \sqrt{1 + \left(\frac{\pi}{4} \cdot \frac{\Delta\Phi_{\text{QPE}}}{\pi/4}\right)^2} \quad \text{(经验公式)}$$

当 $\Delta\Phi_{\text{QPE}} = \pi/4$ 时，$\gamma \approx 1.06$（展宽约 6%）。
...
**物理意义**：距离向分辨单元沿 $\nabla R_{\min}$ 方向延伸 $\rho_{sr}$，该单元在 $h$ 轴上的投影为 $\rho_{sr}\cos\alpha$，在 $x$ 轴上的投影为 $\rho_{sr}\sin\alpha$。这两个量表征距离测量单独贡献的各轴空间定位精度。
...
| 垂直分量（$h$） | $\min\!\bigl(\frac{c}{2B}\cos\alpha,\; \frac{D_a}{2}\sin\alpha\bigr)$ | 距离向与方位向投影的较精细者 |
| 水平分量（$x$） | $\min\!\bigl(\frac{c}{2B}\sin\alpha,\; \frac{D_a}{2}\cos\alpha\bigr)$ | 距离向与方位向投影的较精细者 |
...
| **距离分辨率（垂直投影）** | $\rho_{gr} = c/(2B\sin\theta_{\text{inc}})$ | $\rho_h^{\text{(range)}} = (c/(2B))\cos\alpha$ |
...
| 聚焦深度 | $y_{\max} \approx D_a\cos\alpha\sqrt{2|D|/\lambda}$ |
| 垂直分辨率（距离向投影） | $\rho_h^{\text{(range)}} = (c/(2B))\cos\alpha$ |
| 方位分辨率 | $\rho_{az} = D_a/2$ |
